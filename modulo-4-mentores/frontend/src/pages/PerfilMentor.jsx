  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.body}>
        ...
        {/* Estadísticas */}
        <div style={s.card}>
          <h3 style={{ fontSize:15, fontWeight:700, color:'#1a365d', marginBottom:16 }}>📊 Estadísticas del Mentor</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:12 }}>
            {[
              ['Mentorados activos', stats.mentorados_activos || 0, '#553c9a', '👥'],
              ['Sesiones realizadas', stats.sesiones_realizadas || 0, '#276749', '✅'],
              ['Total evaluaciones', stats.total_evaluaciones || 0, '#d69e2e', '⭐'],
              ['Solicitudes totales', stats.total_solicitudes || 0, '#2b6cb0', '📩'],
            ].map(([l,v,c,ic]) => (
              <div key={l} style={{ ...s.statCard, borderColor:c+'44' }}>
                <div style={{ fontSize:24 }}>{ic}</div>
                <div style={{ fontSize:26, fontWeight:700, color:c, margin:'6px 0' }}>{v}</div>
                <div style={{ fontSize:11, color:'#718096' }}>{l}</div>
              </div>
            ))}
          </div>
          {stats.mentorados_activos >= 3 && (
            <div style={{ marginTop:14, background:'#fffbeb', border:'1px solid #f6e05e', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#744210' }}>
              ⚠️ Este mentor tiene el máximo de mentorados activos (3). Puedes enviar solicitud, pero será considerada cuando haya disponibilidad.
            </div>
          )}
        </div>

          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', maxHeight:150, overflowY:'auto', padding:10, border:'1.5px solid #e2e8f0', borderRadius:8 }}>
            {habilidades.map(h => (
              <button key={h.id_habilidad} 
                ...
                style={{ ...s.badge(form.especialidades.includes(h.id_habilidad) ? '#553c9a':'#718096'), cursor:'pointer', background: form.especialidades.includes(h.id_habilidad) ? '#553c9a':'#f3e8ff', color: form.especialidades.includes(h.id_habilidad) ? '#fff':'#553c9a', border:'none' }}>
                {h.nombre}
              </button>
            ))}
          </div>
            ...
            <form onSubmit={registrarMentor}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                ...
                <div>
                  ...
                  <input type="number" min="1" max="40" required style={s.input} value={regForm.disponibilidad_horas} onChange={e=>setRegForm({...regForm,disponibilidad_horas:e.target.value})} />
                </div>
                <div>
                  <label style={{ fontSize:12, fontWeight:600, color:'#553c9a', display:'block', marginBottom:4 }}>Modalidad*</label>
                  <select style={{ ...s.input }} value={regForm.modalidad} onChange={e=>setRegForm({...regForm,modalidad:e.target.value})}>
                    {['presencial','virtual','ambas'].map(m=><option key={m}>{m}</option>)}
                  </select>
                </div>
              </div>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap', maxHeight:120, overflowY:'auto', padding:10, border:'1.5px solid #d6bcfa', borderRadius:8, background:'#fff' }}>
                  {habilidades.map(h => (
                    <button key={h.id_habilidad} type="button"
                      ...
                      style={{ ...s.tag, cursor:'pointer', background: regForm.especialidades.includes(h.id_habilidad) ? '#553c9a':'#f3e8ff', color: regForm.especialidades.includes(h.id_habilidad) ? '#fff':'#553c9a', border:'none' }}>
                      {h.nombre}
                    </button>
                  ))}
                </div>
                  ...
                  <div key={se.id_sesion} style={{ borderBottom:'1px solid #f0f4f8', paddingBottom:14, marginBottom:14 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12, flexWrap:'wrap' }}>
                      <div>
                        ...
                        <div style={{ fontSize:12, color:'#718096', marginTop:2 }}>👤 {se.mentorado_nombre} {se.mentorado_apellido} · 📡 {se.modalidad} · ⏱ {se.duracion_min}min</div>
                        {se.enlace_virtual && <a href={se.enlace_virtual} target="_blank" style={{ fontSize:12, color:'#553c9a', display:'block', marginTop:2 }}>🔗 {se.enlace_virtual}</a>}

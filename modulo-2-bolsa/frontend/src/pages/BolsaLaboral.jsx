...
export default function BolsaLaboral() {
  ...
  return (
    <div style={s.page}>
      <nav style={s.nav}>
        <span style={{ fontSize:18, fontWeight:700 }}>💼 Bolsa Laboral SGE-UNT</span>
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          {user.rol === 'egresado' && <button style={s.logoutBtn} onClick={() => navigate('/postulaciones')}>Mis Postulaciones</button>}
          {user.rol === 'empresa' && <button style={s.logoutBtn} onClick={() => navigate('/empresa/dashboard')}>Panel Empresa</button>}
        ...
        {recomendadas.length > 0 && (
          <div style={{ marginBottom:28 }}>
            ...
            <div style={{ display:'flex', gap:14, overflowX:'auto', paddingBottom:8 }}>
              {recomendadas.map(o => (
                <div key={o.id_oferta} onClick={() => navigate(`/ofertas/${o.id_oferta}`)}
            ...
            <div style={s.grid}>
              {ofertas.map(o => (
                <div key={o.id_oferta} style={s.card}
                  onClick={() => navigate(`/ofertas/${o.id_oferta}`)}
